...
interface ErrorNote { id: string; title: string; description: string; topic: string | null; banca: string | null; difficulty: string; resolved: boolean; reviewCount: number; wrongCount: number; subject: { name: string }; }

export default function CadernoPage() {
  const [notes, setNotes] = useState<ErrorNote[]>([]);
  const [subjects, setSubjects] = useState<Subject[]>([]);
  const [filter, setFilter] = useState<"all"|"pending"|"resolved">("pending");
  const [subjectId, setSubjectId] = useState(""); const [title, setTitle] = useState(""); const [desc, setDesc] = useState(""); const [topic, setTopic] = useState(""); const [banca, setBanca] = useState(""); const [difficulty, setDifficulty] = useState("Media");
  ...
  const add = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/error-notes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title,description:desc,subjectId,topic,banca,difficulty})});
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gray-950 text-white px-8 py-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {(["pending","all","resolved"] as const).map(f=>(
            <button key={f} onClick={()=>setFilter(f)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${filter===f?"bg-white text-gray-900":"bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[["Total",notes.length,"text-gray-900"],["Pendentes",notes.filter(n=>!n.resolved).length,"text-red-600"],["Resolvidos",notes.filter(n=>n.resolved).length,"text-green-600"],["Críticos",notes.filter(n=>!n.resolved&&n.difficulty==="Alta").length,"text-orange-600"]].map(([l,v,c])=>(
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <form onSubmit={add} className="space-y-4">
            ...
            <div>
              ...
              <input value={title} onChange={e=>setTitle(e.target.value)} required placeholder="Ex: O que é despesa antecipada?" className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"/>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase tracking-wide">Por que errei / Explicação *</label>
              <textarea value={desc} onChange={e=>setDesc(e.target.value)} required rows={3} placeholder="Explique o conceito correto, o que confundiu, como lembrar..." className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none"/>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-gray-500">{n.subject.name}{n.topic&&` • ${n.topic}`}{n.banca&&` • ${n.banca}`}</span>
                  </div>
                ...
                {!n.resolved && (
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button onClick={()=>action(n.id,"review")} title="Revisei, entendi" className="px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-medium transition-colors">Revisei</button>
                    <button onClick={()=>action(n.id,"resolve")} title="Resolver" className="p-2 bg-green-50 hover:bg-green-100 text-green-600 rounded-lg transition-colors"><CheckCircle className="w-4 h-4"/></button>
                  </div>
                )}
              </div>
            </div>
          ))}
          {filtered.length===0&&<div className="text-center py-12 text-gray-400">Nenhum erro {filter==="pending"?"pendente":filter==="resolved"?"resolvido":"registrado"}.</div>}
        </div>
      </div>
    </div>
  );
