...
function errorTypeLabel(value: string | null) {
  ...
  return ERROR_TYPES.find(e => e.value === value) ?? null;
}
...
export default function CadernoPage() {
  ...
  const createFlashcard = async (note: ErrorNote) => {
    ...
    const res = await fetch("/api/flashcards", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ question: note.title, answer: note.description, subjectId: note.subjectId, topic: note.topic ?? "", banca: note.banca ?? "" }),
    });
    if (res.ok) { setFlashcardSuccess(note.id); setTimeout(() => setFlashcardSuccess(null), 3000); }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gray-950 text-white px-8 py-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Registre, revise e resolva seus erros</p>
        </div>
