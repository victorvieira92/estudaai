...
import { TrendingUp, AlertTriangle, CheckCircle, Target, Zap, BookOpen, ArrowRight } from "lucide-react";
...
interface SubjectPriority {
  ...
  // Scores
  difficultyScore: number;   // baseado em erros reais
  priorityScore: number;     // score composto final
  // Classificação
...
function calcPriority(s: any, pendingRevs: number, pendingErrs: number): SubjectPriority {
  ...
  const accuracy = totalQ > 0 ? Math.round((correctQ / totalQ) * 100) : -1;
  const errorRate = totalQ > 0 ? Math.round((wrongQ / totalQ) * 100) : -1;

  // Score de dificuldade real (0-100): quanto mais erro, maior
  // Se não tem dados, assume dificuldade média
  const difficultyScore = totalQ > 0
    ? Math.round(errorRate * 0.7 + (100 - Math.min(s.studyHours * 5, 100)) * 0.3)
    : 50;

  // Score de prioridade composto:
  // Peso edital (30%) + Criticidade (25%) + Dificuldade real (30%) + Pendências (15%)
  ...
  const priorityScore   = Math.round(editalFactor + critFactor + diffFactor + pendingFactor);
  ...
  return {
    ...
    studyHours: s.studyHours ?? 0, totalQuestions: totalQ, correctQuestions: correctQ,
    wrongQuestions: wrongQ, accuracy, errorRate, pendingErrors: pendingErrs,
    pendingReviews: pendingRevs, lastStudyAt: s.lastStudyAt,
    difficultyScore, priorityScore, level, recommendation,
  };
}
...
export default function PrioridadesPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const scored = list.map((s: any) => {
        const pendingRevs = reviews.filter((r: any) => !r.completed && r.pdf?.topic?.subject?.id === s.id).length;
        const pendingErrs = errors.filter((e: any) => !e.resolved && e.subjectId === s.id).length;
  ...
  const sorted = [...subjects].sort((a, b) => {
    ...
    if (sortBy === "accuracy")   return (a.accuracy === -1 ? 101 : a.accuracy) - (b.accuracy === -1 ? 101 : b.accuracy);
    return 0;
  });
  ...
  // KPIs gerais
  const withData = subjects.filter(s => s.totalQuestions > 0);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Total de questões", value: totalQuestions.toString(), sub: "registradas" },
            { label: "Acurácia global", value: totalQuestions > 0 ? `${globalAccuracy}%` : "—", sub: totalQuestions > 0 ? (globalAccuracy >= 70 ? "✓ Boa" : globalAccuracy >= 50 ? "⚠ Regular" : "✗ Crítica") : "sem dados", color: totalQuestions > 0 ? (globalAccuracy >= 70 ? "text-green-600" : globalAccuracy >= 50 ? "text-yellow-600" : "text-red-600") : "text-gray-400" },
            { label: "Matérias críticas", value: critical.toString(), sub: "abaixo de 60%", color: critical > 0 ? "text-red-600" : "text-gray-900" },
            { label: "Matérias em atenção", value: attention.toString(), sub: "entre 60-75%", color: attention > 0 ? "text-yellow-600" : "text-gray-900" },
        ...
        <div className="flex items-center gap-2">
          ...
          {[
            ...
            <button key={key}
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${sortBy === key ? "bg-gray-900 text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gray-400"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {sorted.map((s, i) => {
            ...
            return (
              <div key={s.id} className={`bg-white rounded-2xl border-2 p-6 ${i === 0 && sortBy === "priority" ? "border-gray-900 shadow-md" : "border-gray-200"}`}>
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  {/* Posição + nome */}
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${i === 0 && sortBy === "priority" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600"}`}>
                      {i + 1}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold text-gray-900 text-lg">{s.name}</p>
                        <span className={`flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${cfg.bg} ${cfg.color} border ${cfg.border}`}>
                          <Icon className="w-3 h-3"/> {cfg.label}
                        </span>
                      </div>
                      <p className="text-xs text-gray-400 mt-0.5">
                        {s.lastStudyAt ? `Último estudo: ${new Date(s.lastStudyAt).toLocaleDateString("pt-BR")}` : "Ainda não estudado"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${cfg.bar}`} style={{ width: `${barWidth}%` }}/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mt-4">
                  {[
                    ...
                    { label: "Questões",      value: s.totalQuestions > 0 ? s.totalQuestions.toString() : "—" },
                    { label: "% Acerto",      value: s.accuracy >= 0 ? `${s.accuracy}%` : "—",
                      color: s.accuracy >= 0 ? (s.accuracy >= 70 ? "text-green-600" : s.accuracy >= 50 ? "text-yellow-600" : "text-red-600") : "text-gray-400" },
                    { label: "Erros",         value: s.wrongQuestions > 0 ? s.wrongQuestions.toString() : "—",
                ...
                {/* Recomendação */}
                <div className={`mt-4 flex items-center justify-between p-3 rounded-xl ${cfg.bg} border ${cfg.border}`}>
                  <div className="flex items-start gap-2">
                    <Target className={`w-4 h-4 mt-0.5 shrink-0 ${cfg.color}`}/>
                    <div>
                      <p className={`text-xs font-semibold uppercase tracking-wide ${cfg.color}`}>Recomendação</p>
                      <p className="text-sm font-medium text-gray-800 mt-0.5">{s.recommendation}</p>
                    </div>
                  </div>
                  <Link href="/sessao"
        ...
        <div className="bg-gray-950 text-white rounded-2xl p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {Object.entries(LEVEL_CONFIG).map(([key, cfg]) => (
              <span key={key} className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full ${cfg.bg} ${cfg.color}`}>
