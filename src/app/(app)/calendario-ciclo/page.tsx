...
import { CalendarDays, Settings, Zap, Plus, Trash2, Check, X, Clock } from "lucide-react";
...
export default function CalendarioCicloPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setBlocks(mapped); setDraft(mapped);
      if (mapped.length === 0) setShowConfig(true);
      const saved = parseInt(localStorage.getItem(CYCLE_KEY) ?? "0", 10);
      setCurrentDayIdx(saved);
    }).finally(() => setLoading(false));
  }, []);

  // Dias únicos ordenados
  const cycleDays = [...new Set(blocks.map(b => b.dayOfWeek))].sort((a, b) => a - b);
  const draftDays = [...new Set(draft.map(b => b.dayOfWeek))].sort((a, b) => a - b);
  ...
  const addBlock = (day: number) => {
    setDraft(d => [...d, { dayOfWeek: day, hours: 1, subjectId: subjects[0]?.id ?? null, subjectName: subjects[0]?.name, blockType: "leitura" }]);
  };
  const removeBlock = (day: number, idx: number) => {
    setDraft(d => { const r = [...d]; const db = r.filter(b => b.dayOfWeek === day); const gi = r.indexOf(db[idx]); if (gi === -1) return d; r.splice(gi, 1); return r; });
  };
  const updateBlock = (day: number, idx: number, field: keyof StudyBlock, value: any) => {
    setDraft(d => {
      const r = [...d]; const db = r.filter(b => b.dayOfWeek === day); const target = db[idx]; const gi = r.indexOf(target);
      if (gi === -1) return d;
  ...
  const addDay = () => {
    const usedDays = new Set(draft.map(b => b.dayOfWeek));
    for (let d = 0; d <= 20; d++) { if (!usedDays.has(d)) { addBlock(d); break; } }
  };

  const save = async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/study-blocks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) });
      if (res.ok) {
        const saved = await res.json();
        const mapped = saved.map((b: any) => ({ id: b.id, dayOfWeek: b.dayOfWeek, hours: b.hours, subjectId: b.subjectId, subjectName: b.subject?.name, blockType: b.blockType }));
        setBlocks(mapped); setDraft(mapped); setShowConfig(false);
        localStorage.setItem(CYCLE_KEY, "0"); setCurrentDayIdx(0);
      }
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              {blocks.length > 0 ? `${fmt(totalHours)} por ciclo completo · ${cycleDays.length} dias` : "Configure seus dias de estudo"}
            </p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => { setDraft(blocks); setShowConfig(!showConfig); }}
              ...
              <Settings className="w-4 h-4" />{showConfig ? "Fechar" : "Editar planejamento"}
            </button>
        ...
        {showConfig && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2 flex-wrap gap-3">
              ...
              <button onClick={addDay} className="flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-700 text-white text-sm font-medium rounded-xl transition-colors">
                <Plus className="w-4 h-4" /> Adicionar dia
              </button>
            </div>

            {draftDays.map((day, dayIdx) => {
              const dayBlocks = draft.filter(b => b.dayOfWeek === day);
                ...
                <div key={day} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                      ...
                      {dayTotal > 0 && <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{fmt(dayTotal)}</span>}
                    </div>
                    <button onClick={() => addBlock(day)} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-900 px-2 py-1 rounded-lg hover:bg-gray-100 transition-colors">
                    ...
                    <div className="divide-y divide-gray-50">
                      {dayBlocks.map((block, idx) => (
                        <div key={idx} className="flex items-center gap-3 px-5 py-3">
                          <select value={block.subjectId ?? ""} onChange={e => updateBlock(day, idx, "subjectId", e.target.value || null)}
                          ...
                          <select value={block.blockType} onChange={e => updateBlock(day, idx, "blockType", e.target.value)}
                          ...
                          <button onClick={() => removeBlock(day, idx)} className="text-gray-300 hover:text-red-500 transition-colors">
            ...
            <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-200 p-5">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{draftDays.length} dias no ciclo</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setDraft(blocks); setShowConfig(false); }}
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {subjects.filter(s => blocks.some(b => b.subjectId === s.id)).map(s => {
                ...
                return (
                  <span key={s.id} className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${color?.bg} ${color?.text} ${color?.border}`}>
                    {s.name}
                  </span>
                );
              })}
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {cycleDays.map((day, idx) => {
                ...
                return (
                  <div key={day} className={`bg-white rounded-2xl border-2 p-5 transition-all ${isCurrent ? "border-gray-900 shadow-md" : "border-gray-200"}`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${isCurrent ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600"}`}>
                          {idx + 1}
                        </div>
                        <span className="font-bold text-gray-900">Dia {idx + 1}</span>
                        {isCurrent && <span className="text-xs bg-gray-900 text-white px-2 py-0.5 rounded-full">Atual</span>}
                      </div>
                      ...
                      {dayBlocks.map((block, bi) => {
                        ...
                        return (
                          <div key={bi} className={`rounded-lg px-3 py-2 ${color?.bg ?? "bg-gray-50"} border ${color?.border ?? "border-gray-100"}`}>
                            ...
                            <p className={`text-[10px] mt-0.5 ${color?.text ?? "text-gray-400"} opacity-70`}>
