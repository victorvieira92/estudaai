...
import { CalendarDays, Zap, Plus, Trash2, Check, X, Settings, ChevronLeft, ChevronRight } from "lucide-react";
...
const REVISION_COLOR = { bg: "bg-yellow-50", text: "text-yellow-800", border: "border-yellow-200" };
...
export default function CalendarioCicloPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setSubjects(su);
      const bl = Array.isArray(blocksRaw) ? blocksRaw.map((b: any) => ({
        id: b.id,
  ...
  const removeBlock = (day: number, idx: number) => {
    ...
    setDraft(d => d.filter((b, i) => {
      const dayB = d.filter(x => x.dayOfWeek === day);
      return !(b.dayOfWeek === day && dayB.indexOf(b) === dayBlocks.indexOf(target));
    }));
    // Simpler: filter by reference
    setDraft(d => {
  ...
  const updateBlock = (day: number, idx: number, field: keyof StudyBlock, value: any) => {
    setDraft(d => {
      ...
      if (field === "subjectId") {
        ...
        result[globalIdx] = { ...result[globalIdx], subjectId: value, subjectName: subj?.name };
      } else {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mapped = saved.map((b: any) => ({
          ...
          subjectId: b.subjectId, subjectName: b.subject?.name ?? null, blockType: b.blockType,
        }));
        setBlocks(mapped);
        setDraft(mapped);
        setShowConfig(false);
  ...
  const monday = new Date(today); monday.setDate(today.getDate() + diffToMonday + weekOffset * 7);
      ...
      <div className="bg-gray-950 text-white px-8 py-8">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => { setDraft(blocks); setShowConfig(!showConfig); setSelectedDay(null); }}
        ...
        {!showConfig && (
          <div className="flex items-center justify-between mt-6">
            <button onClick={() => setWeekOffset(w => w - 1)} className="flex items-center gap-1.5 px-3 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition-colors">
              <ChevronLeft className="w-4 h-4" /> Anterior
            </button>
            <div className="text-center">
              <p className="text-sm font-semibold">{weekLabel}</p>
              {weekOffset !== 0
                ? <button onClick={() => setWeekOffset(0)} className="text-xs text-gray-400 hover:text-white underline mt-0.5 transition-colors">Voltar para hoje</button>
        ...
        {showConfig && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-gray-900">Monte seu planejamento semanal</h2>
              <p className="text-sm text-gray-400">Escolha matéria e horas para cada bloco</p>
            </div>
            ...
            {DAY_ORDER.map(day => {
              ...
              return (
                <div key={day} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                  ...
                  {/* Blocos do dia */}
                  {dayBlocks.length === 0 ? (
                    <div className="px-5 py-4 text-sm text-gray-300 italic">Sem estudo — clique em "Adicionar bloco"</div>
                  ) : (
                    <div className="divide-y divide-gray-50">
                      {dayBlocks.map((block, idx) => (
                        <div key={idx} className="flex items-center gap-3 px-5 py-3">
                          ...
                          {/* Remover */}
                          <button onClick={() => removeBlock(day, idx)} className="text-gray-300 hover:text-red-500 transition-colors shrink-0">
            ...
            <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-200 p-5">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatHours(draft.reduce((a, b) => a + b.hours, 0))}
                </p>
              </div>
              <div className="flex gap-2">
                <button
          ...
          <>
            ...
            <div className="grid grid-cols-7 gap-2 mb-6">
              {DAY_ORDER.map((day, i) => {
                ...
                const dayBlocks = blocksByDay.find(d => d.day === day)!;
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-center rounded-2xl border-2 p-2 transition-all cursor-pointer
                      ...
                      ${dayBlocks.totalHours === 0 ? "bg-gray-50" : "bg-white"}
                    `}
                  >
                    ...
                    {dayBlocks.totalHours === 0 ? (
                      <p className="text-[10px] text-gray-300">Folga</p>
                    ) : (
                      <>
                        <div className="w-full space-y-1">
                          {dayBlocks.blocks.map((block, bi) => {
                            const isRevision = block.blockType === "revisao7d" || block.blockType === "revisao14_30d";
                            ...
                            return (
                              <div key={bi} className={`w-full rounded-lg px-1.5 py-1 ${style.bg}`}>
                                <p className={`text-[10px] font-semibold leading-tight truncate ${style.text}`}>{label}</p>
                                <p className={`text-[10px] ${style.text} opacity-70`}>{formatHours(block.hours)}</p>
                              </div>
                            );
                          })}
                        </div>
                        <p className="text-[10px] text-gray-400 mt-2 font-medium">{formatHours(dayBlocks.totalHours)}</p>
                      </>
                    )}
            ...
            {selectedDay !== null && (() => {
              const dayBlocks = blocksByDay.find(d => d.day === selectedDay)!;
              return (
                <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      <h2 className="text-xl font-bold text-gray-900">{DAY_LABEL[selectedDay]}</h2>
                      <p className="text-sm text-gray-500 mt-0.5">{formatHours(dayBlocks.totalHours)} de estudo programadas</p>
                    </div>
                  ...
                  <div className="space-y-3">
                    {dayBlocks.blocks.map((block, i) => {
                      const isRevision = block.blockType === "revisao7d" || block.blockType === "revisao14_30d";
                      const color = block.subjectId ? subjectColorMap.get(block.subjectId) : null;
                      const style = isRevision ? REVISION_COLOR : color ?? SUBJECT_COLORS[0];
                      const label = block.subjectName || BLOCK_TYPES.find(t => t.value === block.blockType)?.label || block.blockType;
                      const typeLabel = BLOCK_TYPES.find(t => t.value === block.blockType)?.label ?? block.blockType;
                      const pct = dayBlocks.totalHours > 0 ? Math.round((block.hours / dayBlocks.totalHours) * 100) : 0;
                      return (
                        <div key={i} className={`rounded-xl border p-4 ${style.bg} ${style.border}`}>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              <p className={`text-lg font-bold ${style.text}`}>{formatHours(block.hours)}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: "Média por dia", value: daysWithStudy > 0 ? formatHours(totalWeekHours / daysWithStudy) : "—" },
              ].map(({ label, value }) => (
